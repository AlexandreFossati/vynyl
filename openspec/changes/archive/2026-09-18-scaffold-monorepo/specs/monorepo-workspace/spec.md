...
The root SHALL expose the scripts `lint`, `typecheck`, `test`, `format`, `build` and `dev`. No script SHALL depend on syntax exclusive to a specific shell, so that they work on Windows, Linux and macOS.
