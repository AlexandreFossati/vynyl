...
The SPA SHALL choose the page by the URL path using the History API: `/` shows the dashboard and any other path shows the not-found page, with a link back to `/`. Internal navigation SHALL happen without reloading the page and SHALL respond to the browser's back and forward buttons.
