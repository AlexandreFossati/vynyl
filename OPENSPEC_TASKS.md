...
1. Criar o change no OpenSpec usando a "Descrição do change" da tarefa como ponto de partida.
2. O agente implementa **apenas** o escopo da tarefa e verifica de verdade (lint, typecheck, testes, execução).
3. O agente entrega o relatório final (formato definido no `CLAUDE.md`) e uma sugestão de mensagem de commit. **Não commita.**
4. O usuário revisa o diff e faz o commit.
5. Se a revisão mudar algum padrão ou decisão, atualizar `PROJECT_GUIDE.md` e a seção **Padrões aprovados** (abaixo) antes da tarefa seguinte.
6. Anotar 3–5 linhas para o `AI.md` (o que funcionou bem/mal nessa tarefa). É mais fácil agora do que reconstruir no final.

**Passo 0 (fora das tarefas, feito pelo usuário)**: `openspec init` (conferir depois que ele não sobrescreveu o `CLAUDE.md`: se escrever nele, manter o nosso conteúdo fora de qualquer bloco gerenciado por ele); decidir se `requirements.pdf`, `DELIVERABLES.md`, `PROJECT_GUIDE.md` e este arquivo entram no repositório (mostram o processo de planejamento); **iniciar a gravação de tela (ou garantir o export do trace de prompts) antes da primeira sessão de código**, pois é um entregável do PDF.

## Visão geral e ordem

| # | Change | Resultado | Prioridade | Checkpoint de revisão |
|---|---|---|---|---|
| T1 | `scaffold-monorepo` | Estrutura completa e vazia + tooling + schema/migration + data set | P0 | Sim: 1º commit |
| T2 | `api-list-products` | **Primeiro endpoint completo** (`GET /api/products`) com toda a fundação e testes | P0 | **Sim: define o padrão da API** |
| T3 | `api-products-remaining` | Demais endpoints obrigatórios seguindo o padrão | P0 | Sim |
| T4 | `api-operability-and-scale` | Rate limit, `/health`, graceful shutdown, singleflight | P1 | Sim |
| T5 | `web-foundation-dashboard` | Design system mínimo + REST client (retry/backoff) + **primeira página** | P0/P1 | **Sim: define o padrão do frontend** |
| T6 | `web-product-pages` | Detalhe, criar, editar, excluir | P0 | Sim |
| T7 | `e2e-and-serving` | Express serve a SPA, `npm start`, Cypress | P0 | Sim |
| T8 | `docs-readme-ai` | README.md e AI.md finais, verificação em clone limpo | P0 | Sim |
| T9 | `optional-endpoints` | Ordenação e categorias | P2 | Só se sobrar tempo |

**Por que esta ordem**: T1 fixa o esqueleto e o banco. T2 e T5 são **tarefas-modelo**: um fatia vertical completa no backend e outra no frontend, para você revisar e ajustar o padrão *antes* de replicá-lo (T3 e T6), o que evita retrabalho em escala. T4 vem depois do CRUD porque o singleflight se aplica às leituras já existentes. T7 integra tudo e só faz sentido com API e SPA prontas. Backend antes do frontend porque a SPA consome a API real; T5 só precisa do endpoint de T2.
...
- Instalar as dependências já decididas no guia (seção 2) para fechar o `package-lock.json` e **validar a instalação em Windows** (em especial `better-sqlite3` e Cypress).
...
**Revisão do usuário**: estrutura de pastas, scripts, dependências instaladas, schema/migration, qualidade do data set. **Commit**: `chore: scaffold monorepo, database schema and dataset`.
...
- Lint, typecheck e testes passam. O agente **executa o servidor e chama o endpoint** (não só testes) e mostra o resultado.

**Revisão do usuário (checkpoint importante)**: separação das camadas, nomes, injeção de dependências, estilo dos testes, formato de erro, organização dos schemas em `shared`, uso de logs. **Tudo que for ajustado aqui vira padrão**: registrar em "Padrões aprovados". **Commit**: `feat(api): add paginated product listing with search`.
...
**Critérios de aceite**: todos os endpoints respondem conforme a seção 7; regras da seção 6 aplicadas; nenhum desvio do padrão de T2 sem aprovação; lint, typecheck e testes passam; o agente exercita os endpoints com o servidor rodando.

**Revisão do usuário**: consistência com T2, cobertura de erros. **Commit**: `feat(api): add product get, create, update and delete endpoints`.
...
**Escopo (inclui)**
- **Rate limit** (`express-rate-limit`) em `/api`, configurável por env (padrão sugerido: 100 req/min/IP), cabeçalhos padrão, 429 `RATE_LIMITED` no formato de erro + `Retry-After`; tratar `trust proxy` via config.
...
- **Documentar no código e no relatório** a limitação de 11.3 (better-sqlite3 é síncrono: em runtime não haverá coalescência real). O agente **não deve** alegar que o singleflight reduz consultas com este driver.

**Fora de escopo**: trocar de driver, cache, métricas, helmet.

**Critérios de aceite**: 429 ocorre ao exceder o limite e volta ao normal depois; `/health` correto; encerramento limpo sem conexões penduradas; testes do singleflight passam; lint/typecheck/testes verdes.

**Commit**: `feat(api): add rate limiting, health check, graceful shutdown and singleflight`.
...
- Roteamento mínimo (rota `/`); escolha da biblioteca validada (compatível com Svelte 5) ou fallback próprio.
...
**Revisão do usuário (checkpoint importante)**: **estética**, granularidade dos componentes, organização Atomic Design, estilo dos testes. Ajustes viram padrão em "Padrões aprovados". **Commit**: `feat(web): add design foundation, http client and product dashboard`.
...
**Critérios de aceite**: CRUD completo pela UI contra a API real; validações client e server refletidas; responsivo em 360/768/1280; acessibilidade básica (labels, foco, `aria-live`); lint/typecheck/testes verdes.

**Commit**: `feat(web): add product detail, create, edit and delete flows`.
...
- **`npm start`**: build de `shared`, `api` e `web`, migrations, seed idempotente e servidor em **http://localhost:3000**. `npm run dev` (API com reload + Vite com proxy). Escolher e documentar no design a forma de build mais simples e multiplataforma (guia, seção 15, item 5).
- **Cypress e2e** contra o servidor real com DB temporário (`DATABASE_PATH`): listar, buscar, paginar, criar, editar, excluir, erro de validação/SKU duplicado; **1 smoke em viewport mobile**. Script `test:e2e` que sobe e derruba o servidor de forma multiplataforma.

**Fora de escopo**: CI (ver guia, seção 15), Docker.

**Critérios de aceite**: em clone limpo, `npm install` + `npm start` abre a aplicação funcional em `localhost:3000` no Windows; `npm run test:e2e` passa; lint/typecheck/testes verdes.

**Commit**: `feat: serve SPA from API, add npm start and Cypress e2e`.
...
**Critérios de aceite**: um dev que não conhece o projeto consegue rodar seguindo só o README; todos os itens de `DELIVERABLES.md` estão atendidos ou explicitamente justificados.

**Commit**: `docs: add README and AI notes`.
...
**Critérios de aceite**: mesmos da API (testes, erros padronizados, padrões aprovados). Atualizar o README com o que foi feito.
